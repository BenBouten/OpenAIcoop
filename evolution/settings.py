"""Configuration constants for the evolution simulation."""

WORLD_WIDTH = 2800
WORLD_HEIGHT = 1600
WINDOW_WIDTH = 1400
WINDOW_HEIGHT = 780

# Colours
WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
GREEN = (124, 252, 184)
RED = (255, 150, 150)
BLUE = (150, 255, 150)
SEA = (194, 252, 250)
BACKGROUND = WHITE

# Simulation tuning parameters
N_LIFEFORMS = 100
N_VEGETATION = 100
N_DNA_PROFILES = 10
MAX_LIFEFORMS = 150
MUTATION_CHANCE = 5
REPRODUCING_COOLDOWN_VALUE = 80

DNA_CHANGE_THRESHOLD = 0.1
COLOR_CHANGE_THRESHOLD = 0.1

GROUP_MIN_NEIGHBORS = 3
GROUP_MAX_RADIUS = 120
GROUP_COHESION_THRESHOLD = 0.35
GROUP_PERSISTENCE_FRAMES = 45
GROUP_MATURITY_RATIO = 0.6

MAX_WIDTH = 20
MIN_WIDTH = 8
MAX_HEIGHT = 20
MIN_HEIGHT = 8
MIN_MATURITY = 100
MAX_MATURITY = 500
VISION_MIN = 10
VISION_MAX = 80
DEGRADE_TIPPING = 3000

FPS = 30

# Time-based tuning (per second rates)
AGE_RATE_PER_SECOND = 6.0
HUNGER_RATE_PER_SECOND = 6.0
ENERGY_RECOVERY_PER_SECOND = 15.0
WOUND_HEAL_PER_SECOND = 30.0
LONGEVITY_HEALTH_DECAY_PER_SECOND = 30.0
EXTREME_LONGEVITY_DECAY_PER_SECOND = 3000.0
HUNGER_HEALTH_PENALTY_PER_SECOND = 3.0
EXTREME_HUNGER_HEALTH_PENALTY_PER_SECOND = 30.0
