...
class Vegetation:
    def __init__(self, x, y, width, height, variant: str = "normal"):
        self.x = x
        self.y = y
    ...
    def decrement_resource(self, amount: float) -> None:
        self.resource -= amount
        if self.resource < 0:
            self.resource = 0

    def regrow(self) -> None:
        # gebruikt global world & environment_modifiers
        biome_modifier = world.get_regrowth_modifier(
            self.x + self.width / 2,
            self.y + self.height / 2,
        )
        growth = (
            self.regrowth_rate
            * environment_modifiers.get("plant_regrowth", 1.0)
            * biome_modifier
        )
