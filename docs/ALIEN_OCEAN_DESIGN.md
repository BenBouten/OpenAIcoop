...
- De startpopulatie vertrekt vanuit één neutrale “common ancestor” met symmetrische vinnen rond het massacentrum en een centrale thruster; optional ray/keel varianten delen exact hetzelfde brein- en mutatieregime en verschillen alleen in module-layout. Elk basisprofiel krijgt kleine willekeurige controllergewichten en wordt bij het klonen licht gemuteerd in lichaam en brein zodat diversiteit uitsluitend uit evolutie voortkomt, niet uit rollen.【F:evolution/simulation/base_population.py†L10-L145】【F:evolution/entities/neural_controller.py†L19-L38】
- Het aantal basisvormen en de populatiegrootte zijn configureerbaar via `INITIAL_BASEFORM_COUNT` en `N_LIFEFORMS` in dezelfde settings/CLI/YAML, zodat je op één plek de startpool kunt aanpassen (standaard 1 baseform).【F:evolution/config/settings.py†L41-L118】【F:configs/default.yaml†L1-L19】
- Spawning plaatst deze clones bewust rond meerdere voedselpatches in gespreide diepte-buckets en vult ontbrekende lagen met synthetische ankers, zodat de startpopulatie meteen op verschillende voedingsdieptes leeft in plaats van uniform random te spawnen.【F:evolution/simulation/bootstrap.py†L97-L199】
